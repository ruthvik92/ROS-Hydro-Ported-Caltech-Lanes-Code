...
//         cout << "\n";
  ...
  {//#ifdef DEBUG_GET_STOP_LINES
    //show the IPM image
    //cvCircle( dbIpmImage, cvPoint(100,125) , 3, CV_RGB(255, 0, 0),-1 );
                 ...
                 LineState* state)
{
  ...
  cout<<"wwwwwwwwwwwwwidddddddth:"<<inImage->height;
  //TODO: smooth image
  //cout<<"breaking here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<"\n";
  //cout<<"inImage"<<inImage;
  CvMat *image = cvCloneMat(inImage);
  //cvSmooth(image, image, CV_GAUSSIAN, 5, 5, 1, 1);

  //SHOW_IMAGE(image, "Input image", 10);

  IPMInfo ipmInfo; //: is defined in InversePerspectiveMapping.hh 

  //state: create a new structure, and put pointer to it if it's null
  LineState newState;   ///LaneDetector.hh defined in 
...
//     #endif
  ...
  list<CvPoint> outPixels;
  list<CvPoint>::iterator outPixelsi;
  cvCircle( image, cvPoint(320,450) , 5, CV_RGB(255, 0, 0),-1 );/////*********************point frm which distance is to b measured.
  mcvGetIPM(image, ipm, &ipmInfo, cameraInfo, &outPixels); // defined in InversePers...Map.cc
...
#warning "Check: Smoothing IPM image"
  ...
  {//#ifdef DEBUG_GET_STOP_LINES
    ...
    cvCopy(ipm, dbIpmImage);
    //show the IPM image
    //cvCircle( dbIpmImage, cvPoint(100,125) , 30, CV_RGB(0, 255, 0),-1 );
    //**SHOW_IMAGE(dbIpmImage, "IPM image123", 10);
  }//#endif
...
#warning "Check this clearing of IPM image for 2 lanes"
  ...
  //show filtered image
  if (DEBUG_LINES) {
    //**SHOW_IMAGE(ipm, "Lane unthresholded filtered123", 10);
  }
...
// 		    state);
  ...
  if(DEBUG_LINES) {//#ifdef DEBUG_GET_STOP_LINES
    ...
    if (stopLineConf->ransacLine || !stopLineConf->ransacSpline)
        for (int i=0; i<(int)dbIpmStopLines.size(); i++)
          mcvDrawLine(dbIpmImage, dbIpmStopLines[i], CV_RGB(255,0,0), 1);
    if (stopLineConf->ransacSpline)
      for (int i=0; i<(int)dbIpmSplines.size(); i++)
        mcvDrawSpline(dbIpmImage, dbIpmSplines[i], CV_RGB(255,0,0), 1);

      
     //**SHOW_IMAGE(dbIpmImage, "Lanes IPM with lines123", 10);
    cvSaveImage("/home/iiith/Desktop/ipm.jpg",dbIpmImage);//********************************************
    //draw lines on original image
    ...
    if (stopLineConf->ransacSpline)
      //cout<<(int)splines->size()<<"splines size";
      
      for (int i=0; i<(int)splines->size(); i++)
      { 
        mcvDrawSpline1(imageClr, (*splines)[i], CV_RGB(255,0,0), 1);
        //**sprintf(str, "%.2f", (*splineScores)[i]);
        mcvDrawText(imageClr, str,
              cvPointFrom32f((*splines)[i].points[(*splines)[i].degree]),
              .5, CV_RGB(0, 0, 255));
      }
    //cvCircle( imageClr, cvPoint(320,450) , 3, CV_RGB(255, 0, 0),-1 );//// *************************8
    //cvCircle( imageClr, cvPoint(370,283) , 20, CV_RGB(255, 0, 0),-1 );
    //cvCircle( imageClr, cvPoint(500,375) , 20, CV_RGB(255, 0, 0),-1 );
    //cvCircle( imageClr, cvPoint(143,9) , 10, CV_RGB(255, 0, 0),-1 );
     //cvCircle( imageClr, cvPoint(348,290) , 10, CV_RGB(0, 255, 0),-1 );
          //cvCircle( imageClr, cvPoint(216,376) , 10, CV_RGB(0,250, 0),-1 );

    //**SHOW_IMAGE(imageClr, "Detected lanes12345", 0);
    //mcvBinarizeImage(imageClr);
    //SHOW_IMAGE(imageClr,"binarized");
    //cout<<"************************";
    //int i,j=450;
  //for(i = 0 ; i<5 ; i++)
  //{
    //cout<<"*********************************************************";
    //cout <<CV_MAT_ELEM(*imageClr, FLOAT_MAT_ELEM_TYPE, i,450);
  //}
    //fprintf(stderr,"*****************IMAGWE**************");
    //cvSaveImage("/home/iiith/Desktop/im2.png",imageClr);*************************************************
    //cvReleaseMat(&image);
    cvReleaseMat(&dbIpmImage);
    //cvReleaseMat(&dbIpmImageThresholded);
 ...
 */
 void mcvInitLaneDetectorConf(char * const fileName,
    LaneDetectorConf *stopLineConf) //LaneDetectorconf struct  defined in LaneDetector.hh
{
  //parsed camera data hre all Lnaesdata from Lanes.conf ll be taken
  LaneDetectorParserInfo stopLineParserInfo; //LaneDetectorParserInfo  struct defined in LaneDetectorOpt.h 
  //read the data
                       ...
                       LineType lineType)
{
  ...
  if(DEBUG_LINES) {//#ifdef DEBUG_GET_STOP_LINES
    ...
    ////SHOW_IMAGE(imClr,"image1",10); ///******************************************88(Lines V Image)
                         ...
                         vector<float> &splineScores, LineState* state)
{
  ...
  for (int i=0; i<(int)boxes.size(); i++) //lines
  {
    ...
    if(DEBUG_LINES) {//#ifdef DEBUG_GET_STOP_LINES
      ...
      ///SHOW_IMAGE(subimageClr,"image2",10);
      //clear
      cvReleaseMat(&subimageClr);
    }//#endif
 ...
 *
 *///*********************************************************mcvDrawSpline clone**********************
 void mcvDrawSpline1(CvMat *image, Spline spline, CvScalar color, int width)
{
  //get spline pixels
  ...
  x0=320;
  y0=450;
  x1=(cvGetReal2D(pixels, 0, 0));
  y1= cvGetReal2D(pixels, 0, 1);
  x2=(cvGetReal2D(pixels, 19, 0));
  y2=cvGetReal2D(pixels, 19, 1);

   dist = ((y2-y1)*x0 - (x2-x1)*y0 + x2*y1 - y2*x1 )/ (sqrt((y2-y1)*(y2-y1)+(x2-x1)*(x2-x1)));
   if(dist<0)
   {
    
    rdist=-(dist);
    cout<<"right distance:"<<rdist<<"\n";
   }
   else
   {
    ldist=dist;
    cout<<"left distance:"<<ldist<<"\n";
   }
   if(ldist<rdist)
   {
    cout<<"ldist:"<<ldist<<"\n";
    cout<<"move to right side"<<"\n";
   }
   else
   {
    cout<<"rdist:"<<rdist<<"\n";
    cout<<"move to left side"<<"\n";
   }
...
void mcvDrawSpline(CvMat *image, Spline spline, CvScalar color, int width)
{
  ...
  if (!pixels)
    return;
  //draw pixels in image with that color ****changed image to image1
   ///*****extra
 ...
 // cout<<"******************************************************************\n";
  ...
  for (int i=0; i<pixels->height-1; i++)
    ...
    // 		color);
     //cout<<"pixel points****************************************\n";
     //cout<<(cvGetReal2D(pixels, i, 0),cvGetReal2D(pixels, i, 1))<<"\n";
     //cout<<"broke here3";
     cvLine(image, cvPoint((int)cvGetReal2D(pixels, i, 0),
                          (int)cvGetReal2D(pixels, i, 1)),
           cvPoint((int)cvGetReal2D(pixels, i+1, 0),
